...
function generateAlerts(
  ...
  if (prediction.isolationRisk >= 60 && !todayAlertIds.includes(`isolation_${today}`)) {
    newAlerts.push({
      ...
      suggestion: 'Even a small connection helps. Open Connection, or reach out to someone you trust - it doesn\'t have to be a big conversation.',
  ...
  if (prediction.riskCategory === 'high' && prediction.trend === 'rising' && !todayAlertIds.includes(`rising_${today}`)) {
    newAlerts.push({
      ...
      suggestion: 'Take a few minutes to journal or check in with yourself about what you need right now.',
      interventionType: 'connection',
      route: '/journal',
      createdAt: new Date().toISOString(),
      isDismissed: false,
