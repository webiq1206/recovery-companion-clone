...
**RevenueCat / subscriptions:** Copy [`.env.example`](.env.example) to `.env` for local native builds. For **TestFlight / App Store** builds, set `EXPO_PUBLIC_REVENUECAT_IOS_API_KEY` on the EAS **`production`** environment (`eas env:create … --environment production`). Verify **Settings → Subscription status** shows **Store key in build: yes** after install. Dashboard setup and QA: [`docs/SUBSCRIPTION_RELEASE_QA.md`](docs/SUBSCRIPTION_RELEASE_QA.md).
