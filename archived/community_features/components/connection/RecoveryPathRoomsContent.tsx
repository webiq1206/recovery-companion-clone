...
function PathSection({ path }: { path: RecoveryPathMeta }) {
  ...
  return (
    <View style={styles.pathBlock} testID={`recovery-path-room-block-${path.id}`}>
      <Pressable
...
export type RecoveryPathRoomsContentProps = {
  /** Optional root test id for smoke tests */
  testID?: string;
};

/**
 * Path cards + copy for Recovery Path Rooms (used on the dedicated stack screen and inline on Connect → Rooms).
 */
export function RecoveryPathRoomsContent({ testID }: RecoveryPathRoomsContentProps) {
  return (
